...
const NotifyRow: React.VFC<NotifyRowProps> = ({ item }) => {
  const renderTransactionName = () => {
    if (item?.transaction?.category) {
      return (
        <div className="flex text-xs text-gray-1 font-bold">{item?.transaction?.category}</div>
      );
    }
    ...
    >
      <div className="flex flex-row items-center">
        <div className="flex flex-row">
          ...
          <div className="flex text-xs text-gray-1 font-regular mx-1">mentioned you in</div>
