...
> = (
  ...
  const titleSelect =
    minMonth !== 0 && maxMonth !== 0
      ? `${dayjs()
          ...
          .format('MMM, YYYY')}`
      : 'Select';
  const applyReadyState = selectedMonths.length > 0 && !isLoadingData ? 'bg-primary' : 'bg-Gray-6';
  ...
  //   setFilter({
  //     props,
  //     periods: getPeriodsByYear(curYear - 1), // Get last year spend
  //   });
  // }, [curYear, props]);
  ...
  const renderIconNextOrLoading = () =>
    isLoadingData ? (
      <Loading width={12} height={12} color="white" className="w-4 h-4 ml-2" />
    ) : (
