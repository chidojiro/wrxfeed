...
export const RoleDrawer = withMountOnOpen()(
  ({ onClose, open, role: roleProp }: RoleDrawerProps) => {
    const isUpdate = !!roleProp;

    const history = useHistory();
    const confirmUpdateDisclosure = useDisclosure();
    const saveSuccessDisclosure = useDisclosure();

    const { role: defaultRole } = useRole(0);
    ...
    }, [assignableCategories, assignableDepartments, assignableVendors, isUpdate, reset, role]);
        ...
        >
          <div className="bg-Gray-12 p-6 border-b border-solid border-Gray-11">
            ...
            <p className="text-sm text-[#6B7280] mt-1">
              By default Base Users can see all transactions. Deselect any property to hide them
              from all team members. Changes to the Base User will affect all team members.
            </p>
          </div>
