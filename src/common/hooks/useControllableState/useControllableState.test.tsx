...
it('should be controlled', () => {
  const { result } = renderHook(() =>
    useControllableState({ value, defaultValue, onChange: mockOnChange }),
  );
...
it('should not set internal state when controlled', () => {
  ...
  act(() => result.current[1]({ internal, external }));

  expect(mockSetStateCallback).not.toBeCalled();
  expect(result.current[0]).toBe(controlledValue);
  expect(mockOnChange).toBeCalledWith(external);
});
