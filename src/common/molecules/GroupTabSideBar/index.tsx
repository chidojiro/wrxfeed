  ...
  return (
    <div className={className}>
      <button
        ...
        className="group-scope flex flex-1 w-full flex-row justify-between py-3 items-center pl-6 pr-6 text-sm rounded-sm"
      >
          ...
          <div className="flex absolute inset-0 group-scope">
            <div className="invisible group-scope-hover:visible absolute -top-9 -right-3">
              <div className="bg-primary px-2 rounded-sm h-6 flex flex-col justify-center">
                <p className="text text-white text-2xs truncate">
                  Follow
                  {group?.name}
                </p>
              </div>
      ...
      {addItemRoute && (
        ...
        >
          <div className="ml-1 group flex flex-1 w-full flex-row hover:bg-Gray-7 justify-between py-3 items-center pl-10 pr-3 text-sm rounded-sm">
            <div className="h-5 w-9" />
            <AddSmallIcon
              className={classNames(
                'flex-shrink-0 h-4 w-4 fill-current path-no-filled stroke-current text-Gray-6 opacity-100 rounded-sm stroke-1',
              )}
              aria-hidden="true"
              width={16}
              height={17}
            ...
            <div className="flex flex-col flex-1 items-start">
              <span className="w-full truncate sm:ml-2 text-left">{`Add ${group?.name}`}</span>
            </div>
          </div>
